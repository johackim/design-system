...
const Modal = ({ title, description, buttons, children, onClose, right, fullscreen, ...props }) => {
    const ref = useRef();
        ...
        <div className={`fixed inset-x-0 px-4 pb-6 inset-0 p-0 flex items-center justify-center z-30 ${isOpen ? 'flex' : 'hidden'}`}>
            <div className="fixed inset-0 transition-opacity">
                ...
                <div className="absolute top-0 right-0 p-8">
                    <XIcon className="h-12 w-12 fill-current text-white cursor-pointer" onClick={closeModal} />
                </div>
            </div>
