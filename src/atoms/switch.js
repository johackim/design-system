...
const Switch = ({ defaultValue, icons, onClick, className }) => {
    ...
    return (
        <button type="button" aria-label="switch" onClick={toggle} className={`${className} !bg-gray-200 dark:!bg-gray-600 relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none`}>
            {icons.length === 2 ? (
